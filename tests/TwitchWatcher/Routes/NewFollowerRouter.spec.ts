...
import NewFollowerRouter from '../../../src/TwitchWatcher/Routes/NewFollowerRouter';
...
describe("handleWebhookCall()", () => {
	...
	test('Should process data', async () => {
		...
		expect(response.json).toHaveBeenCalledWith(new DataMessage({
			desc: `Recieved data under topic: /follow/new`,
			body: request.body,
			processedData: true,
		}));
	});
	...
	test('Should not find a user and fail to process data', async () => {
		...
		expect(response.json).toHaveBeenCalledWith(new ErrorMessage(new Error("Failed to find user with twitch id '1'")));
	});
